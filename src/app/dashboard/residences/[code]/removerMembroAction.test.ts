...
describe("removerMembroAction", () => {
    it("retorna erro e não chama a API quando o usuário não está autenticado", async () => {
        mockGetCurrentUser.mockResolvedValue(null);

        const resultado = await removerMembroAction("AB12CD", 42);

        expect(resultado).toEqual({ success: false, message: "Usuário não autenticado" });
        expect(mockApiFetch).not.toHaveBeenCalled();
    });

    it("em sucesso, envia DELETE com o id do membro e revalida as duas rotas", async () => {
        mockApiFetch.mockResolvedValue(undefined);

        const resultado = await removerMembroAction("AB12CD", 42);

        expect(mockApiFetch).toHaveBeenCalledWith("/residences/AB12CD/members/42", { method: "DELETE" });
        ...
        expect(resultado).toEqual({ success: true, message: "Membro removido da residência." });
    });

    it("repassa a mensagem da API quando ela responde com ApiError", async () => {
        mockApiFetch.mockRejectedValue(new ApiError(403, "Só o proprietário pode remover membros"));

        const resultado = await removerMembroAction("AB12CD", 42);

        expect(resultado).toEqual({ success: false, message: "Só o proprietário pode remover membros" });
